...
const TeamCardSkeleton = () => {
  return (
    <div className="relative max-w-2xl w-full bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-[18px] shadow-md p-6">
      {/* Header Skeleton */}
      <Skeleton className="absolute -top-4 left-1/2 -translate-x-1/2 h-7 w-32 rounded-full" />

      {/* Content Skeleton */}
      <div className="flex flex-col md:flex-row items-center gap-6">
        {/* Image Skeleton */}
        <Skeleton className="w-28 md:w-40 aspect-square rounded-full border border-gray-400 dark:border-gray-600" />
      ...
      {/* Footer Skeleton */}
      <Skeleton className="mt-6 h-4 w-1/3 mx-auto" />
    </div>
  );
};
