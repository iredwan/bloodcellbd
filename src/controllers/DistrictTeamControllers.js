import {
  ...
  GetDistrictTeamByDistrictCoordinatorsUserIdService
...
export const GetDistrictTeamByDistrictCoordinatorsUserId = async (req, res) => {
  try {
    const result = await GetDistrictTeamByDistrictCoordinatorsUserIdService(req, res);
    if (result.status) {
      return res.status(200).json(result);
    } else {
      return res.status(404).json(result);
